...
SelfRepair = false -- true | false (default is true)
NpcRepair = true -- true | false (default is false)
...
FoodKind = "Stuffed Peppers <HQ>" -- "Stuffed Peppers <HQ>" (make sure to have the name of the food IN the "")

UseManual = true
ManalKind = "Commercial Survival Manual"
...
function Repair()
    if NeedsRepair(RepairAmount) and SelfRepair then
        ...
        yield("/wait 0.1")
        if
            GetNodeText("_TextError", 1) == "You do not have the dark matter required to repair that item." and
                IsAddonVisible("_TextError")
         then
            SelfRepair = false
...
function DoGathering()
    yeild("/visland execonce " .. RouteName)
end
